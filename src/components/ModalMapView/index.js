...
class ModalMapView extends React.Component {
  ...
  getFullList10mDatasDetail = (list10mData)=>{
    ...
    for (var i = 36;i>=0;i --){
      ...
      var stringDate = `${newDateObj.getFullYear()}-${pad(newDateObj.getMonth()+1,2)}-${pad(newDateObj.getDate(),2)}T${pad(newDateObj.getHours(),2)}:${pad(minutes,2)}`
      ...
      if(a.length ===0){
        ...
        newArray.push({x:newDateObj.getTime(),y:a[0].Value})
      }
    }
  ...
  getFullList1hDatasDetail = (list1hData)=>{
    ...
    var hours = Math.floor(new Date(newDateObj).getHours())
      var stringDate = `${newDateObj.getFullYear()}-${pad(newDateObj.getMonth()+1,2)}-${pad(newDateObj.getDate(),2)}T${pad(hours,2)}`
  ...
  render() {
    ...
    var new1hArray =[]
     ...
     var label10m = stations.length !==0 && rain10mdata.length !==0 ? `Mã: ${rain10mdata[0].StationID} Trạm: ${stations.find(x=>x.StationID ===rain10mdata[0].StationID).StationName.VN}`:''
